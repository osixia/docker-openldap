# okcupid/openldap
_based on [work](https://github.com/osixia/docker-openldap) by [osixia](https://github.com/osixia)_
...
- [okcupid/openldap](#okcupidopenldap)
...
[Under the hood: okcupid/light-baseimage](#under-the-hood)
	- [Security](#security)
...
- Help new users with [issues](https://github.com/okcupid/docker-openldap/issues) they may encounter
...
```sh
docker run --name my-openldap-container --detach okcupid/openldap:1.5.0
```
...
```sh
docker run -p 389:389 -p 636:636 --name my-openldap-container --detach okcupid/openldap:1.5.0
```
...
docker run \
	...
	okcupid/openldap:1.5.0 --copy-service
...
> [okcupid/openldap-backup](https://github.com/okcupid/docker-openldap-backup)
...
> [okcupid/phpldapadmin](https://github.com/okcupid/docker-phpLDAPadmin)
...
```sh
docker run --hostname ldap.my-company.com --detach okcupid/openldap:1.5.0
```
...
Add --env LDAP_TLS=false to the run command:
	...
	docker run --env LDAP_TLS=false --detach okcupid/openldap:1.5.0

### Multi master replication
Quick example, with the default config.

	#Create the first ldap server, save the container id in LDAP_CID and get its IP:
	LDAP_CID=$(docker run --hostname ldap.example.org --env LDAP_REPLICATION=true --detach okcupid/openldap:1.5.0)
	LDAP_IP=$(docker inspect -f "{{ .NetworkSettings.IPAddress }}" $LDAP_CID)

	#Create the second ldap server, save the container id in LDAP2_CID and get its IP:
	LDAP2_CID=$(docker run --hostname ldap2.example.org --env LDAP_REPLICATION=true --detach okcupid/openldap:1.5.0)
...
To fix that run the container with `--copy-service` argument :
		...
		docker run [your options] okcupid/openldap:1.5.0 --copy-service
...
```sh
docker run --detach okcupid/openldap:1.5.0 --loglevel debug
```

See all command line options:

```sh
docker run okcupid/openldap:1.5.0 --help
```
...
- **LDAP_REPLICATION_HOSTS**: list of replication hosts, must contain the current container hostname set by --hostname on docker run command. Defaults to :
  ...
  - ldap://ldap2.example.org
	...
	If you want to set this variable at docker run command add the tag `#PYTHON2BASH:` and convert the yaml in python:
		...
		docker run --env LDAP_REPLICATION_HOSTS="#PYTHON2BASH:['ldap://ldap.example.org','ldap://ldap2.example.org']" --detach okcupid/openldap:1.5.0
...
### Extend okcupid/openldap:1.5.0 image
...
See complete example in **example/extend-okcupid-openldap**
...
```sh
git clone https://github.com/okcupid/docker-openldap
cd docker-openldap
```
...
okcupid-openldap kubernetes examples are available in **example/kubernetes**

### Under the hood

This image _was_ based on [osixia/light-baseimage](https://github.com/osixia/docker-light-baseimage), which used the following features:
...
However, one of the primary motivations for this fork was the rebase on top of something actively maintained, possibly `debian:slim`.
...
- [osixia/openldap](#osixiaopenldap)
	...
	- [Advanced User Guide](#advanced-user-guide)
		...
		- [Tests](#tests)
		- [Kubernetes](#kubernetes)
[Under the hood: osixia/light-baseimage](#under-the-hood)
	- [Security](#security)
		- [Known security issues](#known-security-issues)
	- [Changelog](#changelog)
...
This image _was_ based on osixia/light-baseimage, which uses the following features:

- **ssl-tools** service to generate tls certificates
- **log-helper** tool to print log messages based on the log level
- **run** tool as entrypoint to init the container environment

)To fully understand how previous base image works take a look at:
